  ...
  if (popups.length) {
      ...
      $(".lesson-text p, .lesson-text td").each(function () {
        ...
        termPopups.forEach((p) => {
          ...
          html = html.replace(re, (match) => {
            ...
            return (
              '<span class="popup-term" tabindex="0" role="button"' +
    ...
    if (diagramPopups.length) {
      ...
      if ($imgCol.length) {
        ...
        const content = diagramPopups
          .map((p) => p.definition)
          .join('<hr style="margin:6px 0">');
        const $btn = $(
          '<button class="popup-info-btn" tabindex="0" aria-label="Diagram info">ⓘ</button>',
        );
        $imgCol.append($btn);
        const pop = new bootstrap.Popover($btn[0], {
          container: "body",
          trigger: "click",
          placement: "left",
          html: true,
        ...
        // Diagram popups explain what's happening — open automatically on load
        setTimeout(function () {
          pop.show();
        }, 500);
      }
    ...
    $(document).on("click", function (e) {
      if (
        !$(e.target).closest(".popup-term, .popup-info-btn, .popover").length
      ) {
  ...
  $(".choice-btn").on("click", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        console.log("Choice recorded:", response);
      },
      error: function (err) {
